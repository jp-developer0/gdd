...
--INSERTAR EN TABLA SUCURSALES	--4 INSERTE Y 3 SELECT , SUPONEMOS QUE SUCURSAL CODIGO TOMA EL VALOR DEL IDENTITY
SET IDENTITY_INSERT GDD2020.SUCURSALES ON
INSERT INTO GDD2020.SUCURSALES (SUCURSAL_CODIGO,SUCURSAL_DIRECCION,SUCURSAL_MAIL , SUCURSAL_TELEFONO )	
SELECT   DISTINCT SUCURSAL_DIR , SUCURSAL_MAIL , SUCURSAL_TELEFONO
FROM gd_esquema.Maestra 
WHERE SUCURSAL_DIR IS NOT NULL

--INSERTAR TABLA PROVEEDOR -- 2 INSERTE Y 1 SELECT , SUPONEMOS QUE SUCURSAL CODIGO TOMA EL VALOR DEL IDENTITY
SET IDENTITY_INSERT GDD2020.PROVEEDOR ON
INSERT INTO GDD2020.PROVEEDOR (PROVEEDOR_CODIGO,EMPRESA_RAZON_SOCIAL)			
SELECT DISTINCT EMPRESA_RAZON_SOCIAL  
FROM gd_esquema.Maestra 
WHERE EMPRESA_RAZON_SOCIAL IS NOT NULL

--INSERTAR EN TABLA FACTURA -- FALTA CALCULAR EL IMPORTE
INSERT INTO GDD2020.FACTURA (FACTURA_FECHA,FACTURA_NRO,NUMERO_CLIENTE,SUCURSAL_CODIGO,COMPRA_NUMERO,IMPORTE)
SELECT FACTURA_FECHA , FACTURA_NRO , cli.CLIENTE_NUMERO , suc.SUCURSAL_CODIGO , COMPRA_NUMERO , 0.0 
--propuesta importe
--select v.precio + est.precio from vuelo v inner join estadia est 
FROM gd_esquema.Maestra 
...
SELECT ma.RUTA_AEREA_CODIGO, ma.RUTA_AEREA_CIU_ORIG, ma.RUTA_AEREA_CIU_DEST
...
--INSERTAR COMPRA   -- usuario no definido????????
INSERT INTO GDD2020.COMPRA(COMPRA_NUMERO,PASAJE_CODIGO,COMPRA_FECHA,PROVEEDOR_CODIGO,ESTADIA_CODIGO,USUARIO_USERNAME)
SELECT ma.COMPRA_NUMERO,ma.PASAJE_CODIGO,ma.COMPRA_FECHA,prov.PROVEEDOR_CODIGO,ma.ESTADIA_CODIGO,'usuario no definido'
FROM gd_esquema.Maestra ma inner join GDD2020.PROVEEDOR prov
on ma.EMPRESA_RAZON_SOCIAL = prov.EMPRESA_RAZON_SOCIAL
